        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Laporan Joki ML</h1>
                    <div class="card mb-4">
                        <div class="card-body">
                            <form method="POST" action="" class="form-inline">
                                <select name="Kat" class="form-control">
                                    <?php
                                        $query    =mysqli_query($conn, "select * from jokiml GROUP BY pesanan ORDER BY pesanan");
                                        while ($data = mysqli_fetch_array($query)) {
                                        ?>
                                    <option value="<?=$data['pesanan'];?>"><?php echo $data['pesanan'];?>
                                    </option>
                                    <?php
                                        }
                                        ?>
                                ...
                                <a class="btn btn-secondary ml-3" href="datajokiml.php">Refresh</a>
                            </form>
                                ...
                                <table class="table table-bordered" id="mauexport">
                                    ...
                                    <tbody>
                                        ...
                                        if (isset($_POST['Kat'])) {
                                            ...
                                            $tamPeg=mysqli_query($conn, "select * from jokiml where setatus='selsai' and pesanan='$unit_kerja' ORDER BY idcus DESC");
                                        } else { 
                                            if (isset($_POST['tangga'])) {
                                            $mulai=$_POST['tang'];
                                            $selsai=$_POST['tangg'];
                                            $tamPeg=mysqli_query($conn, "select * from jokiml setatus='selsai' and tanggal BETWEEN '$mulai' and DATE_ADD('$selsai', INTERVAL 1 DAY) ORDER BY idcus DESC");
                                        } else {
                                            $tamPeg=mysqli_query($conn, "select * from jokiml where setatus='selsai'");
                                        }
                                    }
                                            $i = 1;
